...
const { stdout: git_branch } = await $`git branch --show-current | tr -d '\n'`

const PREFIX = `https://github.com/${repo_author}/${repo_name}/tree/${git_branch}`
...
const items = package_names.map((name) => {
  console.log(chalk.yellow(`${name} added to docs index`))
